...
module.exports = function resolveConfig() {
  ...
  const defaultConfig = {
    ...
    if (!fs.existsSync(path.join(__dirname, "../../config.js"))) {
        terminal.stopAnimation(State.warning, "The configuration was not detected in the root directory, we recommend creating one :) called \"config.js\"");
    ...
    if (typeof configModule != "object") {
        terminal.stopAnimation(State.error, "Failed to load config because an object was not exported");
        process.exit(0);
    }

    if (Array.isArray(configModule)) {
        terminal.stopAnimation(State.error, "Failed to load config because an array was exported instead of a JSON object");
        process.exit(0);
    }
